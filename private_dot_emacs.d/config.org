...
#+begin_src emacs-lisp :tangle no :noweb-ref roam-setup-hook
  (after-init . org-roam-db-autosync-enable)
#+end_src

**** Roam directory
All Roam notes will be stored under =org/roam=.
#+begin_src emacs-lisp :tangle no :noweb-ref roam-variables
...
**** Key mappings
- =C-c o= to enter the interactive selection/creation of a new Roam note to
  write.
- =C-c i= to insert a Roam link at point with interactive selection.
- =C-c m= to toggle the Roam backlinks buffer.
#+begin_src emacs-lisp :tangle no :noweb-ref roam-keys
...
#+begin_src emacs-lisp :tangle no :noweb-ref roam-extra-config
