...
**** Caching
Caching is enabled [[help:projectile-enable-caching][only for the native indexing method]], but the default on all
OSes except Windows is =alien= (see the [[help:projectile-indexing-method][help page]]). Therefore, caching must be
enabled manually.
#+begin_src emacs-lisp :tangle yes
