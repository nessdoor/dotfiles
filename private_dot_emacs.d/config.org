...
**** company-yasnippet backend
     ...
     to all other backends (with the ~:with~ keyword).
     ...
     Just to be safe, define an autoload for the function, so that code can
     implicitly require YAS if it is modifying the backend list and needs to
     fixup it with the YAS cons cells.
     #+begin_src emacs-lisp :tangle yes
       (autoload
...
*** BBDB
    ...
    Activate and make available BBDB inside the =mail= and Gnus interfaces.
    #+begin_src emacs-lisp :tangle yes
      (use-package bbdb
        :ensure t
        :config
        (bbdb-initialize 'mail 'message 'gnus)
    #+end_src
...
**** MUA integration
     ...
     #+begin_src emacs-lisp :tangle yes
         (bbdb-mua-auto-update-init 'mail 'message 'gnus)
     #+end_src
